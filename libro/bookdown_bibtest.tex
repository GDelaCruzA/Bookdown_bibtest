...
\makeatother
\usepackage{xcolor}
\usepackage[left=1.0in, right=1.0in, top=1.25in, bottom=1.25in]{geometry}
\usepackage{color}
\usepackage{fancyvrb}
\newcommand{\VerbBar}{|}
...
Un estilo más, usado con cierta frecuencia por autores de paquetes sobresalientes en el universo de R, usan el ``estilo subrayado'' o de ``serpiente'' (\emph{underscore style} o \emph{snake style}\index{snake style}), en el que cada parte del nombre compuesto está ligado por un guión bajo (\_), de tal manera que el nombre del objeto podría quedar como \texttt{nombre\_muy\_largo\_para\_variable}. \texttt{get\_all\_vars()}, \texttt{collapse\_rows()}, \texttt{divide\_by\_int()}, entre otras funciones, son ejemplos de este estilo.
